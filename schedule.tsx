...
const SLOT_HEIGHT = 60; // px par heure : modifie si tu veux des créneaux plus grands (ex 60)
...
const getCurrentTimePositionDay = (): number => {
  ...
  return (hourIndex + minutePercentage) * 60; // 60 = hauteur d’un créneau dans la vue jour
};

// Ligne pour la vue SEMAINE
const getCurrentTimePositionWeek = (): number => {
  ...
  const hourIndex = hours - 6;
  const minutePercentage = minutes / 60;
  return (hourIndex + minutePercentage) * 50; // 50 = hauteur d’un créneau dans la vue semaine
};
            ...
            {showCurrentTimeLine && (
              <View 
                style={[
                  ...
                  { top: currentTimePosition - 5  } // Offset for header +20
                ]}
              >
  ...
  return (
    <View style={styles.weekDayTimeGrid}>
      {/* NOTE: no inner ScrollView here — the outer week view will handle vertical scrolling */}
        ...
        {HOURS.map((hour) => (
          <View key={hour} style={[styles.weekTimeSlot, { height: SLOT_HEIGHT }]}>
            <View style={styles.weekTimeSlotContent}>
              ...
              {/* Events for this hour (filtered by hour) */}
              {dayEvents
                .filter(event => {
                  const eventStart = new Date(event.startTime);
                  return eventStart.getHours() === hour + 1 ;
                })
                .map(event => {
                  const eventStart = new Date(event.startTime);
                  const eventEnd = new Date(event.endTime);
        ...
        {showCurrentTimeLine && currentTimePosition >= 0 && (
          <View
            style={[
              ...
              { top: currentTimePosition + 56 } // decalage h'1h 
            ]}
          >
...
const renderWeekView = () => {
  ...
  return (
    // Outer vertical ScrollView: scrolls hours (single vertical scroll for whole week)
    <ScrollView style={styles.weekOuterScroll} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={true}>
      <View style={styles.weekTimeGridContainerRow}>
        {/* Time labels column (fixed on the left) */}
        ...
        {/* Horizontal scroll for day columns (headers are inside each day column) */}
            ...
            {weekDates.map((date, dayIndex) => {
              ...
              return (
                <View key={dayIndex} style={styles.weekDayColumn}>
                  {/* Day header inside column so it scrolls horizontally with column */}
                  <View style={[styles.weekDayHeader, isCurrentDay && styles.todayDayHeader]}>
  ...
  return (
    <SafeAreaView style={commonStyles.container}>
        {/* Header */}
  <View style={[styles.header, { alignItems: 'left' }]}>
    <View style={{ flex: 1 }}>
      {/* Titre sur 2 lignes pour libérer de la place au bouton + */}
      <Text style={[commonStyles.subtitle, styles.goalsTitle]}>🕒 Emploi{"\n"}du temps</Text>
    </View>

    <View style={[styles.headerControls, { flexShrink: 0 }]}>
      {/* Boutons / toggles existants */}
...
const styles = StyleSheet.create({
  ...
  weekTimeGridContainerRow: {   // container row for time labels + days
    flexDirection: 'row',
    alignItems: 'flex-start',
  },
	weekDaysColumns: {        //   ChatGPT
    flexDirection: 'row',
    alignItems: 'flex-start',
  },
  ...
  weekDayColumn: {
    width: 110, // largeur d'une colonne de jour : ajuste (100/120) selon préférence
    marginRight: 1,
    position: 'relative',
  },
  weekDayTimeGrid: {
